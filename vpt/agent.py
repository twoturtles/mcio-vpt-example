...
def validate_env(env):
    ...
    # Oops, we have chat action because we're using KarolisRam's minerl fork.
...
def load_vpt(
    ...
    # Updated to use weights_only=True
    agent.policy.load_state_dict(
        th.load(weights_filepath, weights_only=True, map_location=agent.device),
        strict=False,
    )
    agent.reset()
    return agent
