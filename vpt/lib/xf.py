...
#     if tensor is None:
#         print(f"{name: <20}:{DESC_COUNTER[name]}_xf: None")
#     elif not isinstance(tensor, th.Tensor):
#         print(f"{name: <20}:{DESC_COUNTER[name]}_xf:  {tensor}")

#     elif tensor.numel() == 0:
#         print(
#             f"{name: <20}:{DESC_COUNTER[name]}_xf:  {tensor.__class__.__name__}({tensor.shape},{tensor.dtype},{tensor.device})  <numel()==0, stats unavailable>"
#         )
#     else:
#         print(
#             f"{name: <20}:{DESC_COUNTER[name]}_xf:  {tensor.__class__.__name__}({tensor.shape},{tensor.dtype},{tensor.device})  {tensor.float().mean().item()} {tensor[..., 0].float().mean().item()} {tensor[0].float().std().item()} {tensor.float().min().item()} {tensor.float().max().item()}"
#         )
...
class AttentionLayerBase(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.q_layer = MultiscaleLinear(
            x_size, qk_size, name="q", scale=Q_SCALE, **separgs
        )
        ...
        if self.relattn:
            nbasis = 10
            self.r_layer = tu.NormedLinear(x_size, nbasis * attn.h, scale=R_SCALE)
    ...
    def relattn_logits(self, X_bte, T):
        # assert th.are_deterministic_algorithms_enabled()

        # describe("relattn_logits_X_bte", X_bte)
        # describe("relattn_logits_T", T)
        # describe("relattn_logits_r_layer_type", type(self.r_layer))
        # describe("relattn_logits_r_layer_W", self.r_layer.weight)
        # describe("relattn_logits_r_layer_b", self.r_layer.bias)
        R_btn = self.r_layer(X_bte)
        # describe("relattn_logits_R_btn", R_btn)
        R_btn = self.attn.preproc_r(R_btn)
        # describe("relattn_logits_R_btn2", R_btn)
        t = R_btn.shape[1]
        D_ntT = util.bandify(self.b_nd, t, T)
        # describe("relattn_logits_D_ntT", D_ntT)
        extra_btT = th.einsum("btn,ntp->btp", R_btn, D_ntT)
        # describe("relattn_logits_extra_btT", extra_btT)
        return extra_btT
...
class SelfAttentionLayer(AttentionLayerBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ln_x = util.get_norm(norm, x_size)
...
class PointwiseLayer(nn.Module):
    ...
    def __init__(self, x_size, scale, norm, actname="relu", mlp_ratio=2):
        super().__init__()
        s = math.sqrt(scale)
        self.ln = util.get_norm(norm, x_size)
        self.mlp = mlp.MLP(
            insize=x_size,
            nhidlayer=1,
            outsize=x_size,
